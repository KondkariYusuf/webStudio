...
type ProjectRow = {
  id: string;
  title: string;
  latestStaticBuild?: unknown;
  previewImageAsset?: unknown;
  latestBuildVirtual?: unknown;
  domainsVirtual?: unknown;
  [key: string]: unknown;
};

const canRetryWithoutProjectRelation = (error: { message?: string }) =>
  error.message?.includes(
    "Could not find a relationship between 'Project' and '"
  ) === true;
...
const selectProject = async (
  ...
  for (const columns of selectShapes.slice(1)) {
    if (
      result.error === null ||
      canRetryWithoutProjectRelation(result.error) === false
    ) {
      return result;
    }
...
export const loadById = async (projectId: string, context: AppContext) => {
  ...
  const data = await selectProject((columns) =>
  ...
  const {
    latestStaticBuild: rawLatestStaticBuild,
    previewImageAsset = null,
    latestBuildVirtual = null,
    domainsVirtual: rawDomainsVirtual,
    ...project
  } = data.data;

  const latestStaticBuild = Array.isArray(rawLatestStaticBuild)
    ? rawLatestStaticBuild
    : [];
  const domainsVirtual = Array.isArray(rawDomainsVirtual)
    ? rawDomainsVirtual
    : [];
