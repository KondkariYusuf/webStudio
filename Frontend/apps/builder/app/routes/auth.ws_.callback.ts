...
const isMissingOAuthStateError = (error: AuthorizationError) => {
  const message = `${error.message} ${error.cause?.message ?? ""}`.toLowerCase();
  return message.includes("missing state on session");
};
...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  try {
    ...
    if (error instanceof AuthorizationError) {
      ...
      if (isMissingOAuthStateError(error)) {
        debug("Missing oauth state on callback, restarting ws auth flow");
        ...
        session.unset("oauth2:codeVerifier");

        return setNoStoreToRedirect(
          redirect("/auth/ws", {
            headers: {
