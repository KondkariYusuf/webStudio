...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  try {
    ...
    // Validate the redirect_uri shape. Project authorization is validated below
    // using projectId from the requested scope and redirect URI.
    if (
    ...
    if (sessionData) {
      ...
      // redirect_uri: Ensure the redirect_uri targets the same project as requested scope.
      // We avoid strict host string matching here because equivalent builder aliases
      // (for example branch-prefixed hosts) can represent the same project.
      const { projectId: redirectProjectId } = parseBuilderUrl(redirect_uri);

      if (redirectProjectId !== oAuthParams.scope.projectId) {
        debug("redirect_uri does not match the registered redirect URIs");
